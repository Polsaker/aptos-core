...
COPY --link --from=tools-builder /aptos/dist/db-backup /usr/local/bin/db-backup
COPY --link --from=tools-builder /aptos/dist/db-backup-verify /usr/local/bin/db-backup-verify
COPY --link --from=tools-builder /aptos/dist/db-restore /usr/local/bin/db-restore
